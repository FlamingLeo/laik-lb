...
int main(int argc, char *argv[])
{
...
#ifdef DEBUG
    Laik_Partitioning *pm = laik_new_partitioning(laik_Master, world, space, 0);
    laik_switchto_partitioning(dWrite, pm, LAIK_DF_Preserve, LAIK_RO_None);
    laik_get_map_2d(dWrite, 0, (void **)&baseW, &ysizeW, &ystrideW, &xsizeW);
    if (laik_myid(world) == 0)
    {
        ...
        {
            for (uint64_t x = 0; x < xsizeW; ++x)
                printf("%5.1f ", baseW[y * ystrideW + x]);
            printf("\n");
        }
        printf("\n");
    }
    laik_switchto_partitioning(dWrite, pWrite, LAIK_DF_Preserve, LAIK_RO_None);
    laik_get_map_2d(dWrite, 0, (void **)&baseW, &ysizeW, &ystrideW, &xsizeW);
#endif
    ...
    for (; iter < maxiter; iter++)
    {
        ...
        laik_switchto_partitioning(dRead, pm, LAIK_DF_Preserve, LAIK_RO_None);
        laik_get_map_2d(dRead, 0, (void **)&baseR, &ysizeR, &ystrideR, &xsizeR);
        if (laik_myid(world) == 0)
        {
            printf("baseR in iteration %d:\n", iter);
            for (uint64_t y = 0; y < ysizeR; ++y)
            {
                for (uint64_t x = 0; x < xsizeR; ++x)
                    printf("%5.1f ", baseR[y * ystrideR + x]);
                printf("\n");
            }
            printf("\n");
        }
        ...
        if (laik_myid(world) == 0)
        {
            printf("baseW BEFORE LOAD BALANCING after iteration %d:\n", iter);
            for (uint64_t y = 0; y < ysizeW; ++y)
            {
                for (uint64_t x = 0; x < xsizeW; ++x)
                    printf("%5.1f ", baseW[y * ystrideW + x]);
        ...
        if (laik_myid(world) == 0)
        {
            printf("baseW after iteration %d:\n", iter);
            for (uint64_t y = 0; y < ysizeW; ++y)
            {
                for (uint64_t x = 0; x < xsizeW; ++x)
                    printf("%5.1f ", baseW[y * ystrideW + x]);
