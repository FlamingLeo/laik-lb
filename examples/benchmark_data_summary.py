...
def extract_program_name(cmd_formatted, ntasks):
    ...
    if not prog:
        for t in reversed(toks):
            if not t.startswith("-") and t not in ("mpirun", "mpiexec"):
...
def main():
    ...
    sample_cmd = f"mpirun -n {NTASKS_LIST[0]} ./{args.program}"
    ...
    with open(out_csv, "w", newline="") as fh:
        ...
        for nt in NTASKS_LIST:
            cmd_formatted = f"mpirun -n {nt} ./{args.program}"
            ...
            try:
                ...
                print(f"Command timed out for ntasks={nt}")
                lines = [f"TIMEOUT for ntasks={nt}"]
                returncode = -1

            ts = datetime.now().strftime("%Y%m%dT%H%M%S")
            progname = extract_program_name(cmd_formatted, nt)
            logfile = os.path.join(logs_dir, f"{progname}_nt{nt}_{ts}.log")
            ...
            print(f" ntasks={nt}: found task_ids={task_ids} (rows written={len(task_ids)})")
