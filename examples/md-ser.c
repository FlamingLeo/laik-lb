...
/**
 * 2D Molecular Dynamics example for Load Balancing (sequential).
...
// or : https://home.cit.tum.de/~scfl/misc/projects/molsim/docs/ForceCalculation_8h.html
...
{
    char fname[256];
    snprintf(fname, sizeof(fname), "vtk/md_particles_%06ld.vtk", step);
    ...
    for (int i = 0; i < npart; ++i)
    {
        double ke = 0.5 * MASS * (vx[i] * vx[i] + vy[i] * vy[i]);
        fprintf(f, "%g\n", ke);
    }
...
int main(int argc, char** argv)
{
    ...
    // potential
    // note: technically, you should compute an initial potential before the integration loop,
    //       but for the sake of simplicity, this is omitted
    double pot;
