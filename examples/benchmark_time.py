...
exe_arg = sys.argv[1] if len(sys.argv) >= 2 else "./md"
...
re_task = re.compile(
    r"Task\s+(\d+)\s*:\s*work\s*time\s*=\s*([0-9]+(?:\.[0-9]+)?)s\s*,?\s*switch\s*time\s*=\s*([0-9]+(?:\.[0-9]+)?)s\s*,?\s*load\s*balancer\s*time\s*=\s*N/A",
    flags=re.IGNORECASE,
)
...
with open(out_csv, "w", newline="") as csvfile:
    ...
    for idx, ntasks in enumerate(NTASKS, start=1):
        cmd = ["mpirun", "-n", str(ntasks), exe_path, "-o"]
