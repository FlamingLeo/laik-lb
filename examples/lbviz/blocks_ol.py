...
def plot_blocks(times, function_names, block_size=100, out_path="blocks.svg"):
    ...
    first_block_all_tasks = block_sums.sum(axis=0)[0, :]
    order = np.argsort(-first_block_all_tasks)  # largest-first

    # create deterministic colors per function name
            ...
            for idx_pos, func_idx in enumerate(order):
                ...
                if h > 1e-12 and width > 0:
                    rect = Rectangle(
                        ...
                        width,
                        h,
                        facecolor=func_colors[func_idx],
                        edgecolor='none',
                ...
                y_current += h
            # vertical separator
            ax.add_line(plt.Line2D([x_offset + width, x_offset + width], [y_bottom_lane, y_bottom_lane + task_height], color='black', linewidth=0.3))
            x_offset += width
        max_right = max(max_right, x_offset)

    # legend
    legend_order = order
    legend_patches = [Rectangle((0, 0), 1, 1, facecolor=func_colors[i], alpha=1.0) for i in legend_order]
    ...
    ax.set_ylim(-0.1, task_height + 0.1)
    ax.set_yticks([])
    ax.set_xlabel("Time Offset (s)")
    ax.set_ylabel("Overlayed Tasks")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out", default="blocks_ol.svg", help="Output image path")
